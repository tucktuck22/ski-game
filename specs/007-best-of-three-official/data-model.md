...
| Field           | Type                         | Rules                                                                                                                                                                           |
| --------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `attempt_no`    | int                          | Taken when the run starts. `CHECK (attempt_no between 1 and 9)` — a sanity rail, **not** the allowance; the allowance is `officialAttempts` in `data/tuning.json` (FR-245, R10) |
...
| `official_attempts_used`  | **New.** int, `CHECK (>= 0)`. The authority on how many remain (FR-235), compared against `officialAttempts` from tuning. Client-written, like `practice_runs_used` (R2, R10) |
...
| Attempt bound          | —                             | `CHECK (attempt_no between 1 and 9)`      | Sanity rail only. The rule is the tuning value (FR-245, R10) |
...
| Attempts remaining | `officialAttempts - official_attempts_used`, reading the allowance from tuning                                         | FR-231, FR-245 |
| Finished           | `official_attempts_used >= officialAttempts`, or the deadline has passed                                               | FR-239         |
