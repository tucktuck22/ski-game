...
- **The one-run rule is social, not enforced.** Per FR-019, an abandoned official run costs nothing and can be restarted without limit. The product neither prevents a player from bailing out of a bad run nor records that he did (FR-065, withdrawn). This is the correct mechanism for eight friends and the wrong one for strangers. Note the boundary: a run that REACHES a finish or a wipeout is spent the instant it ends and cannot be retaken (FR-017, FR-018), and that half IS enforced, in shared storage.
...
This is accepted rather than engineered away. FR-065 was the chosen response — count abandonment and publish it, so scouting carried a social price instead of a technical one — and it has since been withdrawn by the organizer, who judged the accounting not worth having for eight friends. So the cost above stands, unmitigated and known. Planning MUST NOT introduce a technical block on restarts without amending FR-019.
