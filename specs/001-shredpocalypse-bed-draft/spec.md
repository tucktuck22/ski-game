...
- **FR-017**: System MUST commit the official run's score immediately and irreversibly at run end, including runs that end in a wipeout. **Amended by FR-238 (feature 007): "irreversible" attaches to the ATTEMPT, not to the player's standing. A wipeout still commits and still cannot be retaken, but it no longer ends his competition while attempts remain.**
- **FR-018**: System MUST NOT provide any player-accessible path to retake, edit, or delete a committed official score. **Amended by feature 007: a player has three ATTEMPTS, and no committed attempt may be retaken, edited or deleted. The best of them is his score (FR-232).**
- **FR-019**: ~~An official run whose session ends before the run reaches a finish or a wipeout MUST be discarded in full. No score commits, and the name's official run remains unused and may be started again. There is no limit on how many times an official run may be restarted this way before the deadline.~~ **SUPERSEDED by FR-233 (feature 007), 2026-09-23.** Starting an official attempt now spends it: a session that ends before an end state costs the attempt and posts no score. This is the amendment the Accepted Consequences below demanded before any technical block on restarts, and it closes the unfairness recorded there. See [ADR-0011](../../docs/adr/0011-three-attempts-best-one-counts.md).
...
- **FR-077**: ~~The skier MUST travel at a fixed base speed on the slope, defined in tuning data.~~ **AMENDED 2026-09-10 by feature 006 (FR-227).** The skier's speed on the slope MUST be set by the mountain: produced by gravity along the gradient, opposed by friction and air drag, all defined in tuning data. There is no brake and no input that reduces speed below the floor in tuning data. Slope angle MUST modulate speed within bounds set in tuning data, and the player cannot choose to go slower. _Reason: the original clause described a fixed base speed, which the shipped implementation delivered literally — measured across the entire legal gradient range, speed moved from 2.601 to 2.616, so the mountain had a shape and the simulation did not. The clause permitting slope to modulate speed was always present and is now the mechanism rather than a nudge. See `specs/006-slope-driven-speed/spec.md`._
...
- **FR-030**: ~~Keyboard controls MUST be fully remappable.~~ **STRUCK by the organizer, 2026-09-12, under feature 005's FR-206.** It was never implemented: `saveBindings` has existed in `src/input/keyboard.ts` since this feature and no code has ever called it, so there has never been a way for a player to rebind anything. Asked in the usability review whether to build the bindings UI or drop the requirement, the organizer chose to drop it — the audience is eight known people on their own phones and laptops, and no member of it has asked to rebind. The unread `saveBindings` is removed with it, following the FR-065 precedent that machinery implying a feature which does not exist is worse than no machinery. `loadBindings` stays: it is called, and it is what lets the defaults be read in one place. **This leaves an OPEN DEVIATION from the constitution**, which says without qualification that "Controls MUST be fully remappable" — recorded, with an owner and a date, at [`005/spec.md`](../005-tutorial-and-boundary-rope/spec.md#constitutional-compliance-notes). It needs a decision from the maintainer before this merges, not a patch.
...
- **SC-013**: ~~Every link holder can see, for every player, how many official runs that player abandoned — so that scouting the official course by restarting is possible but never invisible.~~ **WITHDRAWN with FR-065.**
...
- ~~**The one-run rule is social, not enforced.**~~ **REVISED 2026-09-23 by feature 007.** The paragraph below describes the pre-007 rule and is kept as a record. Today: three attempts, starting one spends it (FR-233), and the count is honour-system by decision rather than by omission. Historical text follows. Per FR-019, an abandoned official run costs nothing and can be restarted without limit. The product neither prevents a player from bailing out of a bad run nor records that he did (FR-065, withdrawn). This is the correct mechanism for eight friends and the wrong one for strangers. Note the boundary: a run that REACHES a finish or a wipeout is spent the instant it ends and cannot be retaken (FR-017, FR-018), and that half IS enforced, in shared storage.
...
~~This is accepted rather than engineered away.~~ **CLOSED 2026-09-23 by feature 007.** FR-065 was the chosen response — count abandonment and publish it, so scouting carried a social price instead of a technical one — and it was withdrawn by the organizer, leaving the cost above unmitigated for two weeks.

Feature 007 closes it from the other end: starting an official attempt now spends it (FR-233), so the restart is no longer free for anyone, and every player gets the same three attempts by the rules as written. The amendment this paragraph demanded was made — FR-019 above is superseded, in those terms.

**What remains, stated plainly**: the count is honour-system by the organizer's decision of 2026-09-14, so a player who edits it gets more attempts, exactly as FR-064 lets him post any score. The difference from the old rule is that the product no longer _tells_ him restarting is free. See [ADR-0011](../../docs/adr/0011-three-attempts-best-one-counts.md).

~~**FR-019 also softens the wipeout rule.**~~ **CLOSED 2026-09-23 by FR-233.** "A face-plant on your official run is your score" used to hold only for players who let the face-plant land: quick enough reflexes and a closed tab cost nothing. Bailing now costs the attempt, so the reflex buys a blank attempt rather than a free one. Note that under feature 007 the face-plant itself is also less severe — it costs one attempt of three, not the draft (FR-238).
