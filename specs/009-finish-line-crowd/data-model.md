...
| `stopDistance`               | 200       | He is stopped by this far past the line; a late landing brakes harder to make it, but never in under 40 units.                                                                         |
...
| `crowdFrom`, `crowdTo`       | −160, 480 | The crowd's extent relative to L.                                                                                                                                                      |
| `crowdGapFrom`, `crowdGapTo` | 90, 340   | Nobody stands here: every rider stops in this stretch, so he is never lost in the crowd (FR-275).                                                                                      |
| `crowdSpacing`               | 11        | Units between crowd slots.                                                                                                                                                             |
...
- `braking`: `vx -= brake` with `brake = max(vx²/(2·max(L + stopDistance − x, 40)), vx/max(stopWithinTicks − ticksSinceCrossing, 20))`,
